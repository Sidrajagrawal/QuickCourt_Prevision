...
const AddVenueForm = ({ onClose, onAddVenue }) => {
  ...
  // Asynchronous function to handle file uploads to Cloudinary
  const uploadFileToCloudinary = async (file, type) => {
    const formData = new FormData();
    formData.append('file', file);
    formData.append('upload_preset', 'quickcourt-uploads'); // Replace with your upload preset
    formData.append('folder', 'quickcourt');
    
    // Cloudinary API endpoint
    const url = `https://api.cloudinary.com/v1_1/${'dqqjsp6xo'}/${type}/upload`; // Replace with your Cloudinary cloud name
  ...
  const handleSubmit = async (e) => {
    ...
    if (!accessToken) {
      setUploadStatus({ loading: false, error: 'Authentication failed. Please log in again.' });
      return;
    }
    ...
    try {
      // 1. Upload photos and videos to Cloudinary
      const photoUrls = await Promise.all(photos.map(file => uploadFileToCloudinary(file, 'image')));
      const videoUrl = videos.length > 0 ? await uploadFileToCloudinary(videos[0], 'video') : null;

      // 2. Prepare payload for Django API
      const payload = {
        name: venueName,
        price_per_hour: Number(price),
        operational_hours: `${startHour}:${startMinute} ${startPeriod} - ${endHour}:${endMinute} ${endPeriod}`,
        sports: sports,
        amenities: amenities,
        photos_links: photoUrls,
        video_link: videoUrl,
        description: description,
        // The location data must be sent as nested data for your serializer
        location: {
            ...
            state: locationData.state, // <-- This line was missing
        },
      };

      // 3. Send data to your Django API
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Failed to add venue. Please check your form data.');
      }

      setUploadStatus({ loading: false, success: true, error: null });
      setTimeout(() => {
        onAddVenue(); // Call parent function to refresh venue list
        handleClose();
      }, 1500);
        ...
        <div className="p-6 overflow-y-auto">
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                type="submit"
                className={`px-4 py-2 text-white rounded ${uploadStatus.loading ? 'bg-gray-500' : 'bg-green-500 hover:bg-green-600'}`}
                disabled={uploadStatus.loading}
              >
                {uploadStatus.loading ? 'Uploading...' : 'Save'}
              </button>
            </div>
          </form>
        </div>
